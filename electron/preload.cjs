	...
	contextBridge.exposeInMainWorld('electronAPI', {
	  ...
	  saveCowAgentConfig: (updates) => ipcRenderer.invoke('cowagent:save-config', updates),
	  // ── CanvasKit WASM path ── (for MinoPencil Skia renderer)
	  // Returns a relative path (no leading slash) so fetch() works in both
	  // http:// (Vite dev) and file:// (Electron prod) protocols.
	  // The .wasm file is at public/canvaskit/canvaskit.wasm
	  getCanvasKitWasmDir: () => 'canvaskit/',
	  // Debug logging from renderer to main process console
	  logToMain: (level, ...args) => ipcRenderer.invoke('renderer:log', level, ...args),
	});
