...
function LayerPanelInner() {
  ...
  const handleResizeMouseDown = () => {}; // resize handled by parent ResizablePanel
  ...
  return (
    <div className="bg-card border-r border-border flex flex-col relative w-full">
      <PageTabs />
      <div className="h-8 flex items-center px-3 border-b border-border">
        <span className="text-xs font-medium text-muted-foreground tracking-wider">图层</span>
      </div>
      <div ref={scrollContainerRef} className="flex-1 overflow-y-auto py-1 px-1">
        {children.length === 0 ? (
          <p className="text-xs text-muted-foreground text-center mt-4 px-2">暂无图层</p>
        ) : (
          renderLayerTree(
            children,
            0,
