    ...
    <>
      <div className="flex flex-col">
        <div className="h-8 flex items-center justify-between px-3 border-b border-border">
          <span className="text-xs font-medium text-muted-foreground tracking-wider">页面</span>
          <button
...
function PageContextMenu({
  ...
  const items = [
    ...
    { label: '复制', action: onDuplicate },
    { label: '上移', action: onMoveLeft },
    { label: '下移', action: onMoveRight },
    ...(canDelete ? [{ label: '删除', action: onDelete, danger: true }] : []),
  ];
